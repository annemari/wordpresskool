...
class SSA_Developer_Settings extends SSA_Settings_Schema {
	...
	public function maybe_invalidate_cache( $settings, $old_settings ) {
		if( ! isset( $settings['cache_availability'] ) ) {
			return;
		}

		if ( isset( $old_settings['cache_availability'] ) && $old_settings['cache_availability'] == $settings['cache_availability'] ) {
			return;
		}

		if ( empty( $settings['cache_availability'] ) ) {
