...
class SSA_Availability_Model extends SSA_Db_Model {
	...
	public function filter_where_conditions( $where, $args ) {
		...
		if ( isset( $args['cache_args'] ) ) {
			$where .= ' AND cache_args="'.sanitize_text_field( $args['cache_args'] ).'"';
		}
		...
		if ( isset( $args['contains_period'] ) ) {
			if ( $args['contains_period'] instanceof Period ) {
				$start_date_string = $args['contains_period']->getStartDate()->format( 'Y-m-d H:i:s' );
				$end_date_string = $args['contains_period']->getEndDate()->format( 'Y-m-d H:i:s' );
